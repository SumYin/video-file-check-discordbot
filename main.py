...
f = open('user.json')
user = json.load(f)
cooldown_short_tokens = user.get("cooldown_times").get("short").get("tokens", 1)
cooldown_short_period = user.get("cooldown_times").get("short").get("period", 5)
cooldown_long_tokens = user.get("cooldown_times").get("long").get("tokens", 1)
cooldown_long_period = user.get("cooldown_times").get("long").get("period", 20)
...
async def on_application_command_error(interactions: Interaction, error):
    ...
    # permissions error
    elif isinstance(error, PermissionError): await interactions.send("Permissions error.")
    else: pass
